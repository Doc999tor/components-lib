...
export default class Debts extends React.Component {
  ...
  del = () => {
    ...
    this.props.deleteDebt(
      this.props.customersDebts ? this.state.debt_id : this.state.add_client_id
    );
  };
  ...
  replace = (i, key) => {
    if (
      this.props.debtEdit &&
      this.props.newEditDebt &&
      this.props.debtReplace
    ) {
      this.backButton();
    } else {
    ...
    if (
      ...
      this.setState(
        ...
        () => {
          ...
          this.props.hiddenEmptyDebts && this.props.hiddenEmptyDebts();
        }
      );
    }
  };
  ...
  changeDescription = (e) =>
    this.setState({ description: e.target.value }, () =>
      this.props.getDesc(this.state.description)
    );
  ...
  render() {
    ...
    const sortDebts = this.props.debtsData.sort(
      (a, b) => moment(b.date) - moment(a.date)
    );
    return (
      <div id="debts">
        {(this.props.debtEdit || this.props.debtsData.length > 0) && (
          <div className="debt-header">
            <div className="header-text">
              {config.translations.debts.title}
              <div className="total-debts-wrap">
                <span>{config.data.currency}</span>
                {totalPrice || 0}
              </div>
            </div>
            {this.props.debtEdit && (
              <div className="btn-header" onClick={this.backButton}>
                <div className="btn-header-wrap">
            ...
            <div className="edit">
              ...
              <div className="actions">
                <div className="button-apply" onClick={this.save}>
                  <div
                    className={"img-wrap" + (!this.props.loader ? "" : " spin")}
                  >
                    {!this.props.loader ? (
                      <svg className="img_apply">
                        <use
                          xlinkHref={config.urls.media + "sprite.svg#apply"}
                        />
                      </svg>
                    ) : (
                      <svg className="img_refresh">
                        <use
                          xlinkHref={config.urls.media + "sprite.svg#refresh"}
                        />
                      </svg>
                    )}
                  </div>
        ...
        <div className="debt-body">
          {sortDebts.map((i, k) =>
            this.props.editDebtId === i.id ? (
                ...
                <div className="edit">
                  ...
                  <div className="actions">
                      ...
                      <div className="del-debts" onClick={this.del}>
                        <div className="button-apply" onClick={this.upd}>
                          <div
                            className={
                              "img-wrap" +
                              (!this.props.loaderDel ? "" : " spin")
                            }
                          >
                            {!this.props.loaderDel ? (
                              <img
                                src={config.urls.media + "trash-debts.svg"}
                              />
                            ) : (
                              <svg className="img_delete">
                                <use
                                  xlinkHref={
                                    config.urls.media + "sprite.svg#refresh"
                                  }
                                />
                              </svg>
                            )}
                    ...
                    <div className="button-apply" onClick={this.upd}>
                      <div
                        className={
                          "img-wrap" + (!this.props.loader ? "" : " spin")
                        }
                      >
                        {!this.props.loader ? (
                          <svg className="img_apply">
              ...
              <div
                key={k}
                className={this.props.debtReplace ? "debt-list" : "debt-list"}
              >
                <div className="left-side">
                  <span
                    className={
                      "debt-list-date " +
                      (config.isRTL || config.data.isRTL ? "rtl-side" : "")
                    }
                  >
                    {i.date}
                  </span>
                  <div className="debt-list-name">
                    ...
                    {i.desc && (
                      <div className="debt-list-desc">
                        <span
                          onClick={() => this.showFullDebt(i, i.id)}
                          id={i.id}
                        >
                          {i.desc}
                        </span>
                ...
                <div className="right-side">
                  <img
                    src={config.urls.media + "ic_edit_stroke.svg"}
        ...
        {!this.props.newEditDebt && (
          ...
          >
            <span className="span-edit">
              {config.translations.debts.add_debt_label}
            </span>
